...
// There is a tradeoff in the qBeta parameter between accuracy and response
// speed. In the original Madgwick study, qBeta of 0.041 (corresponding to
...
// robot car! By increasing qBeta (GyroMeasError) by about a factor of
...
// fusion scheme.
static float qBeta = sqrt(3.0f / 4.0f) * GyroMeasError;   // Compute qBeta
...
void MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float deltat){
  ...
  qDot1 = 0.5f * (-q2 * gx - q3 * gy - q4 * gz) - qBeta * s1;
  qDot2 = 0.5f * (q1 * gx + q3 * gz - q4 * gy) - qBeta * s2;
  qDot3 = 0.5f * (q1 * gy - q2 * gz + q4 * gx) - qBeta * s3;
  qDot4 = 0.5f * (q1 * gz + q2 * gy - q3 * gx) - qBeta * s4;
